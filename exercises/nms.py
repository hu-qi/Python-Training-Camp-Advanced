...
def nms(boxes, scores, iou_threshold):
    ...
    while order.size > 0:
        ...
        # 计算 IoU
        ious = np.array([calculate_iou(boxes[i], box) for box in remaining_boxes])
        
        # c. 找到 IoU 小于等于 iou_threshold 的索引
        inds = np.where(ious <= iou_threshold)[0]
        
        # d. 更新 order，只保留那些 IoU <= threshold 的框的索引
        order = order[inds + 1]
